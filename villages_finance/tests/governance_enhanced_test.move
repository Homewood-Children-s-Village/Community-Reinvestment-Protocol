...
fun test_token_weighted_voting(admin: signer, voter1: signer, voter2: signer) {
    ...
    let (metadata, mint_cap) = token::initialize_for_test(&admin, name, symbol);
    ...
    governance::create_proposal(&admin, title, description, threshold, voting_mechanism, action_type, action_data, admin_addr, admin_addr);
    
    governance::activate_proposal(&admin, 0);
    
    // Vote yes - voter1 has 100 tokens, voter2 has 50 tokens
    governance::vote(&voter1, 0, 0);
    governance::vote(&voter2, 0, 0);
    
    // Check status - should pass with 150 total voting power
    let (_, _, status, yes, _, _, _, mechanism) = governance::get_proposal(0, admin_addr);
    assert!(status == 2, 0); // Passed
    assert!(yes == 150, 1); // Token-weighted: 100 + 50
    assert!(mechanism == 1, 2); // TokenWeighted
    
    // Check voting power
    let power1 = governance::get_voting_power(signer::address_of(&voter1), 0, admin_addr);
    assert!(power1 == 100, 3);
    
    let power2 = governance::get_voting_power(signer::address_of(&voter2), 0, admin_addr);
    assert!(power2 == 50, 4);
}
...
fun test_quadratic_voting(admin: signer, voter1: signer) {
    ...
    let name = string::utf8(b"Test Token");
    let symbol = string::utf8(b"TEST");
    let (_, _mint_cap) = token::initialize_for_test(&admin, name, symbol);
    let admin_addr = signer::address_of(&admin);
